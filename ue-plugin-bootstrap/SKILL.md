...
description: 从零搭一个 Unreal Engine C++ 插件,联动 Visual Studio 和蓝图,终点是一个能在蓝图里看到并调用的 UWorldSubsystem 基类。支持 UE 5.0 / 5.1 / 5.2 / 5.3 / 5.4 / 5.5(主流程一致,差异点 skill 内部会自动适配)。适合"刚装好 UE,从零开始"的场景。
---

# UE Plugin Bootstrap Skill

你是一个引导用户从零搭建 UE C++ 插件的助手。本 skill 的目标是**手把手**带用户走完一条完整的链路:
...
**先用 `AskUserQuestion` 问 UE 版本**:

> 你装的是哪个版本的 UE?
> - UE 5.5
> - UE 5.4
> - UE 5.3
> - UE 5.2 / 5.1 / 5.0(选这个的话用户在备注里写具体小版本)
> - 还没装 / 不确定

**版本支持范围**:本 skill 的主流程在 UE 5.0 ~ 5.5 都通用。差异点会在对应步骤自动适配:
- 菜单位置:5.x 系列基本一致
- C++ 标准:UE 5.2+ 默认 C++20,UE 5.0/5.1 默认 C++17(影响极小,模板代码两者都能编)
- `UWorldSubsystem` 基类:5.0 起就有,全系列可用

**版本确认后,问安装路径**(再用一次 `AskUserQuestion`):

> 装在哪?
> - 默认路径 `C:\Program Files\Epic Games\UE_<版本号>`
> - 自定义路径(备注里填)

**没装的处理**:
- 给链接:https://www.unrealengine.com/download
- 提示用户:在 Epic Games Launcher → Library 里安装时,**记一下版本号**(回来告诉我)
- 等用户装完回报,从 1.1 重来

**装好的处理**:
- 把 `Build.bat` 路径推断出来:`<UE 安装路径>\Engine\Build\BatchFiles\Build.bat`
- **存进 memory**:写一个 `ue-bootstrap-paths.md`,记录 UE 版本号、UE 安装路径、Build.bat 路径
- 后续所有步骤里出现"UE 5.3"的地方,都用用户实际版本号替换
...
**VS 与 UE 版本的兼容性**(skill 内部知识,不必都告诉用户,只在用户选择不兼容时提醒):
- UE 5.4 / 5.5:**强烈推荐 VS 2022**(VS 2019 已不再官方支持)
- UE 5.0 ~ 5.3:VS 2019 / VS 2022 都行
- 如果用户用 UE 5.4+ 但选了 VS 2019,提醒"官方不再支持,建议升级 VS 2022"
...
3. 找到用户实际装的版本号(从 memory 里读),点 **Launch**
4. 在弹出的项目浏览器:
   - 左侧选 **Games**(默认)
   - **NEW PROJECT** → 选 **Blank**
   - 右侧设置:
...
> ⚠️ UE 5.4+ 的项目浏览器界面略有调整(选项位置可能在不同 tab 下),如果用户找不到 "Project Type: C++" 这个开关,让他在 Recent Projects 旁边的 tab 里找,或截图给你看。
